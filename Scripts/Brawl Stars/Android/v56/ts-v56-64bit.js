...
try{
    ...
    function assetsChanger(filename, originalfilename) {
        Java.perform(function() {
            ...
            try {
                ...
                // Buffer with a fixed length of 8KB
                ...
                console.log("Read time: " + (Date.now() - startTime) + "ms");
                
                var appDataDir = context.getFilesDir().getAbsolutePath();
                var updateDirPath = appDataDir.slice(0, -("files/".length)) + "/update";
                var updateDir = File.$new(updateDirPath);
                
                if (!updateDir.exists()) {
                    updateDir.mkdirs();
                }
                
                var subDirName = originalfilename.startsWith("csv_logic") ? "csv_logic" : "csv_client";
                var subDirPath = updateDirPath + "/" + subDirName;
                var subDir = File.$new(subDirPath);
                
                if (!subDir.exists()) {
                    subDir.mkdirs();
                }
                
                var outputFilePath = subDir.getAbsolutePath() + "/" + originalfilename.split('/').pop();
                var outputFile = File.$new(outputFilePath);
                
                outputStream = FileOutputStream.$new(outputFile);
                outputStream.write(content);
                console.log("Write time: " + (Date.now() - startTime) + "ms");
                
                console.log(filename + " content has been written to: " + outputFile.getAbsolutePath());
            } catch (e) {
                console.error("Error reading or writing asset: " + e.stack);
            } finally {
                try {
   ...
   // assetsChanger(ModConfig.RangeMod ? "TSMods/effects.csv" : "csv_client/effects.csv", "csv_client/effects.csv")
    ...
    `,
        ...
        Welcome: "Good Stars'a Hoş geldin",
    ...
    const Debug = {
        ...
        createDubugButton() {
            ...
            DebugMenuBase.SetTitle(cache.debugMenu, "GOOD STARS DEBUG")
                ...
                onEnter(args) {
                    ...
                    try {
                        ...
                        if (a1.toInt32() == cache.DebugButtons[5].ins.toInt32()) {
                            FloaterText.show("BAKIMDA - MAİNTEANCE", 0xFFFFFFFF)
                        }
                        ...
                        if (a1.toInt32() == cache.DebugButtons[13].ins.toInt32()) {
                            FloaterText.show("BAKIMDA - MAINTENANCE", 0xFFFFFFFF)
                        }
                ...
                onEnter(args) {
                    ...
                    if (messageType === 20104) {
                        ...
                        setTimeout(()=>{
                            ...
                            NativeDialogManager.ShowDialog("Good Stars v56.274","Debug Menu: gudbs\n","","Close", "");
                        },1000)
                    ...
                    if (messageType === 20103) {
                        ...
                        switch(errcode) {
                            case 8:
                                ...
                                a2.add(184).writePointer('gudbs'.scptr());
                                break;
                        }
                    }
