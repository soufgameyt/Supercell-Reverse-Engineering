...
const Connect = {
	init() {
		Interceptor.attach(Module.findExportByName('libc.so', 'getaddrinfo'), {
            onEnter(args) {
              this.str = args[0] = Memory.allocUtf8String('80.66.87.62'); // ip
              args[1].writeUtf8String('6969'); // port
              //PepperKiller.init();
...
const SetupMessaging = {
    init() {
        const NewLatencyTestsStrPtr = RelocateCString(LatencyTestsStr, 30);
        const NewTidApiTokenShowStrPtr = RelocateCString(TidApiTokenShowStr, 40);
        const NewLobbyInfoWithPlayersOnlineStrPtr = RelocateCString(LobbyInfoWithPlayersOnlineStr, 300);
        var textOpen = "           ";
		...
		const ReceiveMessage = Interceptor.attach(Libg.offset(0x466D00), { // MessageManager::receiveMessage
			onEnter(args) {
                ...
                if (MsgType === 20104) { // LoginOkMessage
                    Interceptor.attach(Libg.offset(0x56FBF8), { // HomePage::startGame Offline Battles
                        onEnter(args) {
                            args[3] = ptr(3);
                        }
                    });
