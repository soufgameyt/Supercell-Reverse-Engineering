...
const START_GAME = 0x16500C;
const GRADIENT = 0x4155B4;
const LINE = 0x53E624;
...
function misc() {
    ...
    Interceptor.attach(base.add(IS_PROD), {
        onLeave(retval) {
            retval.replace(0);
        }
    });
    Interceptor.attach(base.add(GRADIENT), {
        onEnter(args) {
            args[7] = ptr(1);
        }
    });
    const fake_auth_service = Memory.allocUtf8String("button");
    Interceptor.attach(base.add(LINE), {
        onEnter(args) {
            if (args[1].readUtf8String() == "button_line_connect") {
                args[1] = fake_auth_service;
            }
        }
    })
}
function setup() {
    Interceptor.attach(Module.findExportByName('libc.so', 'connect'), {
        ...
        if (ntohs(Memory.readU16(args[1].add(2))) === 9339) {
                ...
                Memory.writeInt(args[1].add(4), inet_addr(Memory.allocUtf8String("188.241.240.136")));
                Memory.writeU16(args[1].add(2), ntohs(7002));
                setupMessaging();
            }
        }
    });
